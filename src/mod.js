...
class AddItems {
    ...
    async postDBLoad(container) {
        ...
        this.mydb = await importerUtil.loadAsync(`${modLoader.getModPath(modFolderName)}database/`);
        ...
        for (const [customID, customItem] of Object.entries(this.mydb.custom_items)) {
            const sptID = customItem.sptID;
            if (this.mydb.custom_items[customID].enable)
    ...
    copyToFilters(itemClone, customID, sptID, enableCompats = true, enableConflicts = true) {
        ...
        const sptIDs = [];
        for (const customItem of Object.values(this.mydb.custom_items))
